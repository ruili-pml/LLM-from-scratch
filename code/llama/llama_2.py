...
class LLaMA(nn.Module):
    ...
    def forward(
        ...
        if self.rope_cache is None:
            self.rope_cache = self.build_rope_cache(idx) # [block_size, head_dim / 2, 2] 
        if self.mask_cache is None:
            self.mask_cache = self.build_mask_cache(idx) # [1, 1, block_size, block_size]

        if input_pos is not None:
            rope = self.rope_cache.index_select(0, input_pos) # [1, head_dim / 2, 2]
            mask = self.mask_cache.index_select(2, input_pos) # [1, 1, 1, block_size]
...
class CausalSelfAttention(nn.Module):
    ...
    def forward(
        ...
        if kv_cache is not None:
            cache_k, cache_v = kv_cache # each has shape [B, n_head, max_seq_length, head_size]
            ...
            k = cache_k.index_copy(2, input_pos, k) # fancy version of cache_k[:, :, input_pos, :] = k
            v = cache_v.index_copy(2, input_pos, v)
            kv_cache = k, v

        # causal self-attention (spelt out but slow version)
